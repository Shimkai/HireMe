...
export const createJobSchema = Joi.object({
  ...
  eligibility: Joi.object({
    minCGPA: Joi.number().allow('').optional().min(0).max(10),
    allowedCourses: Joi.array().items(Joi.string()),
    maxBacklogs: Joi.number().allow('').optional().min(0),
    yearOfCompletion: Joi.array().items(Joi.number()),
    minTenthPercentage: Joi.number().allow('').optional().min(0).max(100),
    minTwelfthPercentage: Joi.number().allow('').optional().min(0).max(100),
  }),
  ctc: Joi.object({
    min: Joi.number().allow('').optional().min(0).required(),
    max: Joi.number().allow('').optional().min(0).required(),
